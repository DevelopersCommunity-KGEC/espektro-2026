...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        if (session?.user?.role === 'ticket-issuer') {
            router.replace('/dashboard/manual-tickets');
            return;
        }
        ...
        if (session?.user?.role && session.user.role !== 'security' && session.user.role !== 'ticket-issuer') {
            fetchData();
        }
    }, [session, router]);

    if (loading || (session?.user?.role === 'security' || session?.user?.role === 'ticket-issuer')) {
        return (
