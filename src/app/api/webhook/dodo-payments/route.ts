    ...
    if (metadata?.eventId && metadata?.userId) {
      ...
      try {
        const teamMembers = metadata.teamMembers
          ? JSON.parse(metadata.teamMembers)
          : undefined;
        await verifyPayment(
          metadata.eventId,
          ...
          metadata.couponCode, // was referralCode in previous code but metadata in booking-actions says 'couponCode'
          { userId: metadata.userId, email: metadata.email || "" }, // Can we get email from metadata?
          metadata.referrerUserId, // attribution
          teamMembers,
        );
      } catch (err) {
