...
export default async function EventDetailsPage({
  ...
  if (session?.user && !event.allowMultipleBookings) {
    await dbConnect();
    if (id === "season-pass" || event.type === "season-pass") {
      // Check for Season Pass Booking via Origin (New System)
      const originTicket = await Ticket.findOne({
      ...
      if (originTicket) {
        existingTicket = JSON.parse(JSON.stringify(originTicket));
      } else {
        // Fallback: Check if the user already has tickets for ALL fest-day events (Legacy System or Indiv. Buys)
